...
from helper import get_data, standardise_data, partial_accuracy_callable, generate_neurons
...
# Standard scaling works better for the NN than normalisation
...
architecture = generate_neurons(max_depth=10, average_neurons_per_layer=100) + generate_neurons(10, 20) + generate_neurons(5, 200)
print(architecture)
parameter_grid = {
    'hidden_layer_sizes': Categorical(architecture),
    'tol': Continuous(1e-4, 1e-3, distribution='uniform')
}
...
evolved_nn = MLPClassifier(activation='relu', solver='lbfgs', random_state=101, alpha=0.006)
...
def optimise_neural_network():
    ...
    # Save the results
    output_file = open("lbfgs_nn_best_params.json", "w")
